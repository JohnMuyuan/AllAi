...
import type { OfficialQuotaMap } from "./quota";
 ...
 * 主进程每 5 分钟问一次，界面上统计行、通知也会顺带触发。
 *
 * 存 `~/.allai/quota-history.json`：
...
const KINDS = ["claude", "chatgpt", "grok"] as const;
...
function historyFile() {
  return path.join(dataDir(), "quota-history.json");
}

export function readQuotaHistory(): QuotaHistory {
  try {
    const parsed = JSON.parse(fs.readFileSync(historyFile(), "utf8")) as QuotaHistory;
