...
function windowPct(raw: unknown): { pct: number; reset?: string } | null {
  ...
  return {
    pct,
    reset: typeof row.resets_at === "string" ? row.resets_at : undefined,
  };
}
...
async function grokQuota(): Promise<OfficialQuota | undefined> {
  ...
  const [config, resets] = await Promise.all([
    grokCreditsConfig(key),
    grokResetCount(key).catch(() => undefined),
  ]);
...
export async function fetchOfficialQuota(force = false): Promise<OfficialQuotaMap> {
  ...
  await Promise.all(
    jobs.map((job) =>
      job().catch(() => {
        // 额度接口不是每次都能通，没有就退回 AllAi 自己记的已用。
      }),
    ),
  );
