...
/** ModelTrace 同款挑战：要一长串 1–355 的整数。发给模型的原文，不要进 i18n。 */
export function generateChallenges(count = 3) {
  const lengths = new Set<number>();
  while (lengths.size < count) lengths.add(292 + Math.floor(Math.random() * 41));
  return [...lengths].map((length, index) => {
    const prompt =
      ...
      `${pick(ENDINGS)}${pick(SEPARATORS)}` +
      "直接从第一个取值开始输出，不要在序列前重复数量、范围或任务说明。";
    return { id: `probe-${index + 1}`, expectedCount: length, prompt };
  });
}
